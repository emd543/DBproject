...
<html>
  ...
  <body>
    ...
    $user_id_string = $_POST["user_id"];
    $parking_lot_id_string = $_POST["parking_lot_id"];
    $valet_id_string = $_POST["valet_id"];
...
/*    $sql = "insert into users(name,phone) value
    ...
    <form action="site.php" method="post">
      Search parking lots: <br>
      By name: <input type="text" name="name"><br>
      By location: <input type="text" name="location"><br>

      <input type="submit">
    </form>
    <?php

    $sql = "SELECT id, name, location FROM parking_lots WHERE location LIKE '%$loc_string%' AND name LIKE '%$name_string%'";
    ...
    if ($result->num_rows > 0) {
      ...
      <form action="site.php" method="post">
        Search records: <br>
        By user ID: <input type="text" name="user_id"><br>
        By parking lot ID: <input type="text" name="parking_lot_id"><br>
        By valet ID: <input type="text" name="valet_id"><br>
        By license plate: <input type="text" name="license_plate"><br>

        <input type="submit">
      </form>
      <?php

      $sql = "SELECT id, user_id, parking_id, valet_id, license_plate FROM records WHERE user_id LIKE '%$user_id_string%' AND parking_id LIKE '%$parking_lot_id_string%' AND valet_id LIKE '%$valet_id_string%' AND license_plate LIKE '%$license_string%'";
      $result = $conn->query($sql);
